...
abstract contract ERC4626Test is ERC4626Prop {
    ...
    function setUpVault(Init memory init) public virtual {
        ...
        for (uint256 i = 0; i < N; i++) {
            ...
            // shares
            uint256 shares = uint256(init.share[i]);
            vm.assume(shares > 0.01 ether);
            IMockERC20(_underlying_).mint(user, shares);
            _approve(_underlying_, user, _vault_, shares);
            vm.prank(user);
            IERC4626(_vault_).deposit(shares, user);
            // assets
            uint256 assets = uint256(init.asset[i]);
            vm.assume(assets > 0.01 ether);
    ...
    // convert
    //
    function test_convertToShares(Init memory init, uint128 assets) public virtual {
        setUpVault(init);
    ...
    function test_convertToAssets(Init memory init, uint128 shares) public virtual {
        setUpVault(init);
        address caller1 = init.user[0];
        address caller2 = init.user[1];
    ...
    function test_previewDeposit(Init memory init, uint128 assets) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        address other = init.user[2];
        assets = uint128(bound(assets, 0, _max_deposit(caller)));
        _approve(_underlying_, caller, _vault_, type(uint128).max);
        prop_previewDeposit(caller, receiver, other, assets);
    }

    function test_deposit(Init memory init, uint128 assets, uint128 allowance) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        assets = uint128(bound(assets, 0, _max_deposit(caller)));
    ...
    function test_previewMint(Init memory init, uint128 shares) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        address other = init.user[2];
        shares = uint128(bound(shares, 0, _max_mint(caller)));
        _approve(_underlying_, caller, _vault_, type(uint128).max);
        prop_previewMint(caller, receiver, other, shares);
    }

    function test_mint(Init memory init, uint128 shares, uint128 allowance) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        shares = uint128(bound(shares, 0, _max_mint(caller)));
    ...
    function test_previewWithdraw(Init memory init, uint128 assets) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        address owner = init.user[2];
        address other = init.user[3];
        assets = uint128(bound(assets, 0, _max_withdraw(owner)));
        _approve(_vault_, owner, caller, type(uint128).max);
        prop_previewWithdraw(caller, receiver, owner, other, assets);
    }

    function test_withdraw(Init memory init, uint128 assets, uint128 allowance) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        address owner = init.user[2];
    ...
    function testFail_withdraw(Init memory init, uint128 assets) public virtual {
        setUpVault(init);
        address caller = init.user[0];
        address receiver = init.user[1];
        address owner = init.user[2];
    ...
    function _max_deposit(address from) internal virtual returns (uint256) {
        if (_unlimitedAmount) return 100_000_000 ether;
