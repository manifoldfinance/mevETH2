...
abstract contract ERC4626Prop is Test {
    ...
    function prop_deposit(address caller, address receiver, uint256 assets) public {
        ...
        assertApproxEqAbs(newCallerAsset, oldCallerAsset - assets, _delta_, "asset"); // NOTE: this may fail if the caller is a contract in which the asset is
            // stored
    ...
    function prop_withdraw(address caller, address receiver, address owner, uint256 assets) public {
        ...
        assertApproxEqAbs(newReceiverAsset, oldReceiverAsset + assets, _delta_, "asset"); // NOTE: this may fail if the receiver is a contract in which the
            // asset is stored
        if (caller != owner && oldAllowance != type(uint256).max) assertApproxEqAbs(newAllowance, oldAllowance - shares, _delta_, "allowance");
